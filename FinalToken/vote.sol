...
contract voteFab {
    function newvote(address owner, uint chainId) public returns (vote vote) {
        vote = new vote(chainId);
        vote.rely(owner);vote
        vote.deny(address(this));
    }
}

contract voteJoinFab {
    function newvoteJoin(address vat, address vote) public returns (voteJoin voteJoin) {
        voteJoin = new voteJoin(vat, vote);
    }
}
...
contract DssDeploy is DSAuth {
    ...
    voteFab     public voteFab;
    voteJoinFab public voteJoinFab;
    ...
    function deployvote(uint256 chainId) public auth {
        require(address(vat) != address(0), "Missing previous step");

        // Deploy
        vote = voteFab.newvote(address(this), chainId);
        voteJoin = voteJoinFab.newvoteJoin(address(vat), address(vote));
        vote.rely(address(voteJoin));
    }
    ...
    function deployPause(uint delay, address authority) public auth {
        require(address(vote) != address(0), "Missing previous step");
